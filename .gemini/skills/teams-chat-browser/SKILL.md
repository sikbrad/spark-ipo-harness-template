---
name: teams-chat-browser
description: (Fallback) Microsoft Teams 채팅(DM/그룹) **본문** — playwright-cli + 내부 REST(`/api/chatsvc/.../messages`) 방식. 사용자가 명시적으로 "브라우저로", "playwright로", "teams-chat-browser" 라고 한 경우 또는 신규 `/teams-chat`(Graph API)이 실패한 경우에만 호출. 일반적인 Teams 채팅 본문 조회의 default는 `/teams-chat`.
---

# Microsoft Teams 채팅 본문 조회 — Browser fallback (REST 직접 호출)

> **이 스킬은 fallback 입니다.** 일반적인 Teams 채팅 본문 조회는 `/teams-chat` (MS Graph API, 브라우저 불필요) 가 default. 다음 경우에만 본 스킬을 사용:
> - 사용자가 명시적으로 "브라우저로", "playwright로", "teams-chat-browser" 라고 지시
> - `/teams-chat` (Graph) 호출이 401/403/네트워크 오류 등으로 실패한 경우 (Graph 토큰 만료·권한 누락 등)
> - Graph가 노출하지 않는 필드(예: `48:notifications` Activity 가상 conversation)가 필요한 경우
