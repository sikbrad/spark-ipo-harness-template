---
name: daily-collect
description: 하루치 활동 로그를 여러 데이터 소스(Notion 트리·Notion Jot DB·Raindrop·MS Teams 채팅·MS Teams 채널·Slack·Google Calendar·Gmail/Outlook·Google Drive·Jira·Confluence·포탈 Feedback·포탈 공지사항/릴리즈노트)에서 모아 `data/daily/<YYYY-MM-DD>/raw/`에 raw 저장하고, AI가 직접 raw를 읽고 분석한 일별 `summary.md`를 작성. "오늘 뭐했는지", "일일 로그", "데일리 정리", "daily activity 모아줘", "내가 무슨일했는지 정리해줘" 등 일별 활동 회고 요청 시 사용. 키워드 매칭이나 API 자동 요약 금지 — LLM이 raw를 한 건씩 읽고 직접 사람말로 정리.
---
...
└── <YYYY-MM-DD>/
    ...
    │   ├── portal-feedback.json         # 포탈 Feedback/Notification 변경분 + active feedback snapshot
    │   └── portal-notices.json          # 포탈 공지사항/릴리즈노트 변경분 + release note snapshot
...
### 11) Portal Feedback — 포탈 피드백/알림

포탈 `Feedback`은 AX팀이 만든/운영하는 포탈 업무 입력이다. 나이트루틴과 일별 회고에서는 사용자의 회사 업무 기록과 같은 업무 신호로 취급한다.

```bash
node proc/lib/portal_feedback_collect.mjs 2026-05-18
```

저장 위치: `data/daily/<day>/raw/portal-feedback.json`. 구조:

```
{
  "dailyFeedback": [...],        # KST day에 created/updated/notificationSentAt이 걸친 Feedback
  "activeFeedback": [...],       # 현재 NEW/REVIEWED/DOING/PRE_DEPLOY feedback snapshot
  "feedbackNotifications": [...] # KST day의 FEEDBACK Notification
}
```

운영 규칙:

- 기본 DB는 포탈 repo `.env`의 `DATABASE_URL_REMOTE`이며 없으면 `DATABASE_URL`을 사용한다.
- 수집기는 `BEGIN READ ONLY` 안에서 SELECT만 수행한다.
- 실패해도 루틴을 멈추지 말고 `source-errors.json`와 summary에 실패 이유만 남긴다.
- 테스트성 피드백은 테스트/검증 업무로, 실제 사용자 요청은 업무 피드백/다음 액션으로 정리한다.

### 12) Portal Notices — 포탈 공지사항/릴리즈노트

포탈 공지사항의 릴리즈노트는 AX팀의 배포/운영 산출물이다. 일별 회고에서는 사용자의 회사 업무 기록과 같은 업무 신호로 취급한다.

```bash
node proc/lib/portal_notice_collect.mjs 2026-05-18
```

저장 위치: `data/daily/<day>/raw/portal-notices.json`. 구조:

```
{
  "dailyNotices": [...],         # KST day에 created/updated가 걸친 Notice
  "releaseNoteSnapshot": [...],  # 제목/본문상 릴리즈노트로 보이는 non-deleted Notice
  "publishedNotices": [...]      # 현재 게시 중인 Notice snapshot
}
```
...
- 수집기는 `BEGIN READ ONLY` 안에서 SELECT만 수행한다.
- 실패해도 루틴을 멈추지 말고 `source-errors.json`와 summary에 실패 이유만 남긴다.
- 릴리즈노트는 “한 일/산출물/배포 커뮤니케이션”으로 정리한다.
...
4. **포탈 Feedback** — AX팀이 접수/검토/처리한 피드백과 상태 변화. 사용자 업무 맥락과 같은 신호로 본다.
5. **포탈 공지사항/릴리즈노트** — 릴리즈노트, 점검공지, 게시 상태 변화. AX팀 산출물로 본다.
6. **다음 액션** — raw에서 발견된 follow-up. 명시되지 않았으면 빈칸.
...
| Portal Feedback DB 접근 실패 | `portal-feedback.json`은 생략하거나 error payload만 남기고 summary에 한 줄 표시. |
| Portal Notice DB 접근 실패 | `portal-notices.json`은 생략하거나 error payload만 남기고 summary에 한 줄 표시. |
