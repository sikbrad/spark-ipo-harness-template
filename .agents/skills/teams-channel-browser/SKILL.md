---
name: teams-channel-browser
description: (Fallback) Microsoft Teams 채널 게시물(thread+replies) — playwright-cli + 내부 REST(`/api/csa/.../containers/{cid}/posts`) 방식. 사용자가 명시적으로 "브라우저로", "playwright로", "teams-channel-browser" 라고 한 경우 또는 신규 `/teams-channel`(Graph API)이 실패한 경우에만 호출. 일반적인 Teams 채널 게시물 조회의 default는 `/teams-channel`.
---

> **이 스킬은 fallback 입니다.** 일반적인 Teams 채널 게시물 조회는 `/teams-channel` (MS Graph API, 브라우저 불필요) 가 default. 사용자가 "브라우저로/playwright로/teams-channel-browser" 명시 지정 또는 Graph 호출이 401/403/네트워크 등으로 실패한 경우에만 본 스킬 사용.
