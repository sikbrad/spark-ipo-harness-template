...
kmsg mcp-server
```
Claude Code/Codex/Gemini의 `.mcp.json`에 등록하면 LLM이 직접 `kmsg_read`/`kmsg_send`/`kmsg_chats` 도구 호출 가능. 본 스킬과 중복이라 둘 다 쓸 필요는 없음 — 본 스킬은 명시적 CLI 호출 패턴, MCP 모드는 자율 LLM 도구 사용 패턴.
