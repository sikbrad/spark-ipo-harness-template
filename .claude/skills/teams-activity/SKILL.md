...
`browser-harness` + `agent_helpers.py`의 `teams_*` helper로 Teams web의 두 가지 좌측 사이드바 패널을 긁는다:
1. **Activity 피드**(종 아이콘) — @멘션·답글·반응 등 알림 스트림
2. **Chat 리스트**(채팅 아이콘) — DM/그룹챗 목록과 마지막 메시지 (미답·미열람 분석용)
...
| 역할 | 셀렉터 |
|---|---|
| 좌측 앱바 Chat 버튼 | `button[aria-label*="Chat"]` (한글 UI: `button[aria-label*="채팅"]`) |
| 좌측 chat row | `[role="treeitem"]` (단, x좌표 ~50–360 범위, height 30–130 필터 필요) |

Chat row의 `innerText`는 **개행(`\n`) 구분자**로 구성:
```
[Unread]          # (옵션) 미열람 배지 — 있을 때만 첫 줄
{name}             # DM이면 사람 이름, 그룹이면 그룹명
{time}             # "4:19 PM" / "5/6" 등
{last_msg}         # 마지막 메시지 — DM이면 본문, 그룹이면 "발신자: 본문"
```

마지막 줄이 `You: ` 또는 `나: ` 로 시작하면 본인이 마지막 발화 → 답을 기다리는 입장.
