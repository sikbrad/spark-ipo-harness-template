...
**권장 운영 사이클** (북마크가 늘어났을 때):
```
1) /raindrop          → 새 북마크 dump
2) /raindrop-infer    → 새 북마크 본문 요약 (infer md 생성)
3) /raindrop-retag    → 새 북마크에 3태그 + AI 메모
```
이전 사이클에서 `no-infer` 로 skip된 raindrop도 2)에서 infer가 채워지면 3)에서 자동 처리됨.
...
```bash
# raindrop.io 서버 측 spot-check (로컬 mirror 말고 진짜 API)
python3 -c "
import sys; sys.path.insert(0, 'proc/lib')
from raindrop_api import RaindropClient
c = RaindropClient.from_env()
for rid in [1112329686, 1145916659, 1719182215]:
    r = c.raindrop_retrieve(rid).get('item', {})
    print(rid, r.get('tags'))
    print(r.get('note')); print()
"
...
for p in Path('data/raindrop/dump/raindrops').glob('*.json'):
    ...
    if '--- ai memo' in (r.get('note') or ''):
        for t in r.get('tags') or []: c[t] += 1
for t, n in c.most_common(30): print(f'{n:4d}  {t}')
...
- raindrop API: 120 req/min (≈0.51s/req).
- OpenAI: gpt-4o-mini, MIN_INTERVAL_SEC=0.6s/req — **이쪽이 dominant**.
- **실측**: 1281건 풀런 = 2902초 ≈ **48분**. throughput ≈ 26 raindrop/min.
- **비용 실측**: gpt-4o-mini 1281건 ≈ **$0.4** 미만.
