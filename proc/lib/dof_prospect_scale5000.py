...
def write_cards(prospects: list[Prospect], limit: int | None = None) -> None:
    ...
    for idx, prospect in enumerate(selected, start=1):
        ...
        acquisition_path = prospect.tags.get("acquisition_path", "공개 출처에서 업체명, 이메일, 전화, 주소, 업종 근거를 확인")
        site_business_summary = prospect.tags.get("site_business_summary", prospect.dof_fit)
        product_keywords = prospect.tags.get("product_keywords", "")
        product_line = f"- 제품/영업 키워드: {product_keywords}" if product_keywords else ""
...
## 데이터 취득 및 취급품목
- 데이터 취득 경로: {acquisition_path}
- 취급품목/사이트 요약: {site_business_summary}
{product_line}
