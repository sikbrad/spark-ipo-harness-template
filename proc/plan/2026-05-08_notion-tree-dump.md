...
3. `python proc/lib/notion_dump.py dump --root e0a658bf0f8d4e6384c6903940c7e7a9 --out data/notion/dump`
...
| 단계 | 스크립트 | 입력 | 출력 |
|------|----------|------|------|
| A | `notion_dump.py` | Notion API | `data/notion/dump/` (raw) **+ inline asset download** → `data/notion/assets/` |
| B | `notion_hydrate.py` | A의 `pages/` + `blocks/` | `data/notion/hydrated/<id>.json` (page + 모든 blocks 재귀 머지) |
| C | `notion_render_md.py` | B의 hydrated | `data/notion/md/<path>/<title>.md` (frontmatter + MD) |
