...
| 단계 | 스크립트 | 입력 | 출력 |
|------|----------|------|------|
| A | `notion_dump.py` | Notion API | `output/notion-dump-2026-05-08/` (raw, 현재) |
| B | `notion_hydrate.py` | A의 `pages/` + `blocks/` | `output/notion-hydrated-2026-05-08/<id>.json` (page + 모든 blocks 재귀 머지) |
| C | `notion_render_md.py` | B의 hydrated | `output/notion-md-2026-05-08/<path>/<title>.md` (frontmatter + MD) |
| X | `notion_assets.py` | A의 `blocks/` (image/file blocks) + `pages/` (cover/icon) | `output/notion-assets-2026-05-08/<page_id>/<filename>` |
...
- **B || X** : 둘 다 A의 raw 만 read. 다른 폴더 write. **동시 실행 가능**.
- **C** depends on B. 추가로 C가 asset 링크를 로컬 경로로 rewrite 하려면 X 결과 필요 → C는 B+X 둘 다 끝나야 안전.
- **A 자체는 단일 프로세스** — Notion API rate limit 공유라 분기시켜도 토큰 분할 외엔 가속 안 됨.

### URL 만료 리스크 (X)

Notion-hosted 첨부 URL은 발급 후 ~1시간 만료. raw dump가 1h+ 걸리면 초반 페이지의 image URL은 X 실행 시점엔 이미 만료됨. 두 가지 옵션:

1. **분리 단계 (current 설계)**: dump 끝나는 즉시 X 실행. 일부 만료 이미지는 fallback (Notion에 재요청해서 fresh URL 받기 — `pages_retrieve` 다시 → block에 새 URL 박힘. 또는 영향 받은 page 만 raw refetch).
2. **인라인**: X를 dumper에 통합, blocks 가져오자마자 image/file 즉시 다운. 만료 zero. 하지만 단계 분리 깨짐.

당분간 (1) 로 가되, 만료 발생 빈도 보고 (2) 로 옮길지 결정.
